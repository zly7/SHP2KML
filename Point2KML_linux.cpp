...
{
    vector<string> strMatchNameArray = Split(argv[1], '.');
    string szXmlFileName = strMatchNameArray[0] + ".kml";
    ...
    {
        string strLine5;
        while (getline(inFile5, strLine5)) // line does not include the newline character of each line
        {
    ...
    {
        vector<string> strMatchFileArray = Split(str, ',');

        Waypoint test_Aero;

        test_Aero.lon = atof(strMatchFileArray[0].c_str());
        test_Aero.lat = atof(strMatchFileArray[1].c_str());
        test_Aero.alt = atof(strMatchFileArray[2].c_str());

        test_Aero.heading = atof(strMatchFileArray[3].c_str());
        test_Aero.pitch = atof(strMatchFileArray[4].c_str());
